...
def collect_experience(tf_env, agent, meta_agent, state_preprocess,
                       ...
                       store_context,
                       disable_agent_reset, current_steps):
  ...
  def increment_current_step():
    return current_steps.assign_add(1)
  ...
  def reset_current_steps():
    return current_steps.assign(0)
  ...
  step_cond = agent.step_cond_fn(state, action,
                                 transition_type,
                                 environment_steps, num_episodes, current_steps)
  reset_episode_cond = agent.reset_episode_cond_fn(
      state, action,
      transition_type, environment_steps, num_episodes, current_steps)
  ...
  increment_current_step_op = tf.cond(step_cond, increment_current_step, reset_current_steps)
  ...
  increment_op = tf.group(increment_step_op, increment_current_step_op, increment_episode_op,
                          increment_reset_op)
...
def train_uvf(train_dir,
  ...
  current_steps = tf.Variable(0, dtype=tf.int64, name='current_steps')

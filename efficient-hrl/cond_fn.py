...
def env_transition(agent, state, action, transition_type, environment_steps,
  ...
  Returns:
    ...
    not RESTARTING
  """
  del agent, state, action, num_episodes, environment_steps, current_steps
...
def every_n_steps(agent,
  ...
  Args:
    ...
    environment_steps: Number of steps performed by environment.
    num_episodes: Number of episodes.
    n: Return true once every n steps or when episode should terminate based on cartpole.
  Returns:
  ...
  """
  del agent, action, transition_type, num_episodes
  pole_fell = tf.logical_or(state[2] > 12, state[2] < -12)
  cart_gone = tf.logical_or(state[0] > 2.4, state[0] < -2.4)
  cond = tf.logical_or(tf.logical_or(pole_fell, tf.equal(tf.mod(current_steps, n), 0)), cart_gone)
  return cond
...
def false_fn(agent, state, action, transition_type, environment_steps,
  ...
  """
  del agent, state, action, transition_type, environment_steps, num_episodes, current_steps
